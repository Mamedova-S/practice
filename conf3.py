...
def get_all_links(html):
    soup = bs(html, 'lxml')
    ads = soup.find('div', class_='notice-list').find_all('div', class_='notice-item')

    all_links = []

    for index, ad in enumerate(ads):
        link = ad.find('div', class_='notice-item-title').find('a').get('href')
...
def get_page_data(html):
    ...
    try:
        title = soup.find('main', class_='content ').find('h1', class_='content-page-header-inner').text
    except Exception:
        title = ''
    # print(title)
    # try:
